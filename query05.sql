create index nbhd_index on azavea.neighborhoods using GiST(geog);
...
select
	neighborhood,
	round(cast((some_accessibility::float/(some_accessibility + no_accessibility + unknown_accessibility)) as numeric), 3) as metric
from nbhd_data
)

select * from final_table order by metric desc;